...
extern  int cjpegMain (int argc, char **argv);
extern  int djpegMain (int argc, char **argv);
...
Java_com_andnux_core_utils_ImageUtils_cjpegMain(JNIEnv *env, jclass type, jint argc,
                                                jobjectArray argv) {
    int ac = argc;
    int len = (*env)->GetArrayLength(env, argv);
    printf("\n c-stringArray :");
    char **argvs = malloc(sizeof(char*) * len);
    int i = 0;
    for (i = 0; i < len; i++) {
        jobject obj = (*env)->GetObjectArrayElement(env, argv, i);
        jstring str = (jstring) obj;
        const char * szStr = (*env)->GetStringUTFChars(env, str, 0);
        argvs[i] = (char *) malloc(sizeof(char*) * 1012);
        printf("%s", szStr);
        strcpy(argvs[i],szStr);
    }
    int ret =  cjpegMain(ac, argvs);
    for (int i = 0; i < len; i++) {
        free(argvs[i]);
    }
    free(argvs);
...
JNIEXPORT jint JNICALL
Java_com_andnux_core_utils_ImageUtils_djpegMain(JNIEnv *env, jclass type, jint argc,
                                                jobjectArray argv) {
    int ac = argc;
    int len = (*env)->GetArrayLength(env, argv);
    printf("\n c-stringArray :");
...
JNIEXPORT jint
JNICALL
Java_com_andnux_core_utils_ImageUtils_compress(JNIEnv *env, jclass type,
